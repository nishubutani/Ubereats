...
class UberLinkSpider(scrapy.Spider):
    name = 'taiwan_category'
    allowed_domains = ['example.com']
    i=1
    start_urls = ['http://www.example.com/']

    db_name = dbc.db_name
    cur = db_name['citylink_TW']
    cur1 = db_name['categorylink_TW']
    ...
    def start_requests(self):
        ...
        for row in urls:
            ...
            link = 'http://api.scraperapi.com/?api_key=df1a32d04b794153ad1c51a152bf520f&keep_headers=false&premium=false&url=' + links
            if '?' in links:
                links=links.split('?')[0]
            ...
            yield scrapy.FormRequest(url=link,callback=self.parse,dont_filter=True,headers=header,meta={'link':links})
    ...
    def parse(self,response):
        ...
        if response.status == 200:
            cat = response.xpath("//*[contains(text(),'檢視全部')]/@href").getall()
            for i in cat:
                if 'category' in i:
...
if __name__ == '__main__':
    ...
    execute('scrapy crawl taiwan_category'.split())#-a start=0 -a end=4000
