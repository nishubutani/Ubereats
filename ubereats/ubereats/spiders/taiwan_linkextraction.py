...
class UberLinkSpider(scrapy.Spider):
    ...
    db_name = dbc.db_name
    cur = db_name['categorylink_TW']
    cur1 = db_name['link_TW']
            ...
            if response.status == 200:
                ...
                for div in divs:
                    try:
                        ...
                        try:
                            delivery_fee = div.xpath('.//span[contains(text(),"費用")]//text()').get(default='')
                            # delivery_fee = delivery_fee.split(" ")[-1].strip()
                            if delivery_fee == '':
                                delivery_fee = div.xpath('.//div[contains(text(),"費用")]//text()').get(default='')
                        ...
                        try:
                            delivery_time = div.xpath('.//sapan[contains(text(),"分鐘")]//text()').get(default='')
                        except Exception as e:
                            delivery_time = ''
                            print(e)
...
if __name__ == '__main__':
    ...
    execute('scrapy crawl taiwan_extraction -a a=0 -a b=10000'.split())
