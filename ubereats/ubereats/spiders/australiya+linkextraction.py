...
class UberLinkSpider(scrapy.Spider):
    name = 'australiya_linkextraction'
    ...
    db_name = dbc.db_name
    cur = db_name['categorylink_AU']
    cur1 = db_name['link_AU']
    ...
    def start_requests(self):
        ...
        for row in urls[int(self.a):int(self.b)]:
            ...
            # url = 'http://api.scraperapi.com/?api_key=ec46668b42a54199b95f52ab3d9ba6f8&keep_headers=true&premium=false&url=' + links
            url =  links
            if '?' in links:
                links = links.split('?')[0]
            header = {
                ...
                for div in divs:
                    try:
                        ...
                        try:
                            ...
                            if delivery_fee == '':
                                delivery_fee = div.xpath('.//div[contains(text(),"Delivery Fee^")]//text()|.//span[contains(text(),"Delivery Fee^")]//text()').get(default='')
...
if __name__ == '__main__':
    ...
    execute('scrapy crawl australiya_linkextraction -a a=0 -a b=10000'.split())
